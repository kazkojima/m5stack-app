...
// For roter identifier
float filter_gain = 0.1f;
...
void imu_task(void *arg)
{
    ...
    RotorIy RI (filter_gain, 1.0e-3, 1.0e-6);
    float c, pi, pj, pk;
...
#endif
                RI.UpdateIMU (gx, gy, gz, ax, ay, az, c, pi, pj, pk);
#if 1
                if ((count % 100) == 0) {
                    RI.Show ();
...
#error "bad ROTATION_YAW value"
            ...
            if (trim_mode) {
                ...
                //printf("mx: %f my: %f mz: %f\n", mx, my, mz);
                mx_adj = (1 - 0.02)*mx_adj + 0.02*mx;
                my_adj = (1 - 0.02)*my_adj + 0.02*my;
                mz_adj = (1 - 0.02)*mz_adj + 0.02*mz;
#if 1
                if (count < IN_CALIB)
                    ;
